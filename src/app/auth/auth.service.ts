...
export class AuthService {
    ...
    signup(email: string, password: string) {
        return this.http.post<AuthResponseData>(
            'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyC9h6sLGDJQgYNtpRpgbbpAcogu7dnbDkM',
            {
                email: email,
                password: password,
    ...
    private handleAuthentication(
        ...
        // this.user.next(user);
        this.store.dispatch(new AuthActions.AuthenticateSuccess({ email: user.email, userId: user.id, token: user.token, expirationDate: new Date(new Date().getTime() + expiresIn * 1000) }));
